...
class HBNBCommand(cmd.Cmd):
    ...
    def do_show(self, args):
        """Usage: show BaseModel 1342-3453-2311"""
        args = args.split()
        if len(args) == 0:
        ...
        if args[0] not in HBNBCommand.valid_classes:
            ...
            return
        objects = storage.all()
        for objs_id in objects.keys():
            if objs_id == args[1] and args[0] in str(type(objects[objs_id])):
                print(objects[objs_id])
                return
        print("** no instance found **")

    def do_destroy(self, args):
        """Usage: destroy BaseModel 1342-3453-2311"""
        args = args.split()
        if len(args) == 0:
            print("** class name missing **")
            return
